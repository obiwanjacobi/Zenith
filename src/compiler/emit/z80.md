...
| `LD L1, L2 + LD H1, H2 + LD L2, L1 + LD H2, H1` | Delete | |

## Function Stack Frame

- Normal instruction selection does not change SP (no push/pop).
- When function calls return SP is unchanged.
...
- SP calling function
- Stack Frame
  - Offset: 3 (ptr-init) [4, 5, 6]
  - Offset: 0 (arr-init) [1, 2, 3]
  - SP points to last byte stack frame
- (lower memory addresses)

If no local variables are stack-bound, no prologue and epiloque is generated.

Function Prologue:

- Move SP down to make room for stack frame

Function Body:

- Stack is indexed (offset) from SP upward.

Function Epilogue

- Move SP up to cleanup stack frame
